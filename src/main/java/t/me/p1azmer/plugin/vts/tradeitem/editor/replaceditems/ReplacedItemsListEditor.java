package t.me.p1azmer.plugin.vts.tradeitem.editor.replaceditems;
...
public class ReplacedItemsListEditor extends EditorMenu<VTSPlugin, TradeItem> implements AutoPaged<ItemStack> {

    public ReplacedItemsListEditor(@NotNull TradeItem tradeItem) {
        ...
        this.addCreation(EditorLocales.CREATE_ITEM, 2).setClick((viewer, event) -> {
            ...
            if (cursor != null && !cursor.getType().isAir()) {
                if (tradeItem.getReplaceItems().size() >= 2) {
                ...
                tradeItem.getReplaceItems().add(cursor);
    ...
    public List<ItemStack> getObjects(@NotNull Player player) {
        return new ArrayList<>(this.object.getReplaceItems());
    }
    ...
    public ItemClick getObjectClick(@NotNull ItemStack reward) {
        return (viewer, event) -> {
            if (event.isRightClick() && event.isShiftClick()) {
                this.object.getReplaceItems().remove(reward);
                this.save(viewer);
                return;
            }
